...
export function LanguageControl({ welcome = false }: { welcome?: boolean }) {
  ...
  return <div className={`language-control${welcome ? ' language-control--welcome' : ' icon-button'}`} title={msg('language.label')}>
    ...
    <select
      aria-label={msg('language.label')}
      value={interfaceLanguage()}
      onPointerDown={event => event.currentTarget.parentElement?.setAttribute('data-focus-source', 'pointer')}
      onKeyDown={event => event.currentTarget.parentElement?.removeAttribute('data-focus-source')}
      onBlur={event => event.currentTarget.parentElement?.removeAttribute('data-focus-source')}
      onChange={event => void setInterfaceLanguage(event.target.value as 'zh-CN' | 'en')}
    >
